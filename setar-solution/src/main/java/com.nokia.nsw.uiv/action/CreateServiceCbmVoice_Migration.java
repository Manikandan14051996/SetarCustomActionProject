...
import com.nokia.nsw.uiv.request.CreateServiceCBMBulkRequest;
...
public class CreateServiceCbmVoice_Migration implements HttpAction {
    ...
    @Autowired
    private CreateServiceCbmVoice_Migration self;
    ...
    public Object doPost(ActionContext actionContext) throws Exception {
        ...
        CreateServiceCbmVoiceBulkRequest bulkRequest =
                (CreateServiceCbmVoiceBulkRequest) actionContext.getObject();

        List<CreateServiceCbmVoiceRequest> requests = bulkRequest.getServices();

        for (CreateServiceCbmVoiceRequest request : requests) {
    ...
    public CreateServiceCbmVoiceResponse singleReqprocess(CreateServiceCbmVoiceRequest request) throws BadRequestException, NotFoundException {
        ...
        if (!cpeOpt.isPresent()) {
            throw new NotFoundException("CPE device not found with cbmName:" + cpeDeviceName);
        }
        ...
        if (cbmName.length() > 100) {
            throw new BadRequestException("CBM name value too long");
        }

        if (rfsName.length() > 100) {
            throw new BadRequestException("RFS name value too long");
        }


        if (cfsName.length() > 100) {
            throw new BadRequestException("CFS name value too long");
        }

        if (subscriptionName.length() > 100) {
            throw new BadRequestException("Subscription name value too long");
        }

        // name length checks
        if (subscriberNameString.length() > 100) {
            throw new BadRequestException("Subscriber name value too long");
        }
        ...
        try {
            ...
            Customer subscriber = subscriberRepository.findByDiscoveredName(subscriberNameString)
                    .orElseGet(() -> {
                        ...
                        try {
                            ...
                            s.setDiscoveredName(subscriberNameString);
                        } catch (AccessForbiddenException | BadRequestException |
                                 ModificationNotAllowedException e) {
            ...
            try {
                ...
                throw new RuntimeException("Persistence error while updating subscriber: " + e.getMessage());
            }
...
//                    subsProps.put("voipPackage1", request.getServicePackage());
                ...
                throw new RuntimeException("Persistence error while updating subscription: " + e.getMessage());
            }
            ...
            if (productNameStr.length() > 100) {
                throw new BadRequestException("Product Name Value too Long");
            }
            ...
            LogicalDevice cbmDevice = cbmDeviceRepository.findByDiscoveredName(cbmName)
                    .orElseGet(() -> {
                        ...
                        if (request.getCbmGatewayMac() != null)
                        ...
                        if (request.getCbmManufacturer() != null)
            ...
            if ("Voice".equalsIgnoreCase(request.getProductSubtype())) {
                ...
                try {
                    ...
                    throw new RuntimeException("Persistence error while updating CPE device: " + cpeDeviceName + e.getMessage());
                }
            }
