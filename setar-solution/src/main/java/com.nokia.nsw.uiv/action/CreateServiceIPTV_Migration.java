        ...
        for (CreateServiceIPTVRequest request : bulk.getServices()) {
            ...
            try {
                ...
                try {
                    ...
                    CreateServiceIPTVResponse response=new CreateServiceIPTVResponse("400", ERROR_PREFIX + bre.getMessage(),
                            Instant.now().toString(), "", "");
                ...
                if (isSubscriberExist.get() && isSubscriptionExist.get() && isProductExist.get()) {
                    ...
                    CreateServiceIPTVResponse response= new CreateServiceIPTVResponse("409", "Service already exist/Duplicate entry", Instant.now().toString(), subscriptionName, "ONT" + request.getOntSN());
                    responses.add(response);
                    continue;
                }
                ...
                if (optCFS.isPresent()) {
                    ...
                    CreateServiceIPTVResponse response=new  CreateServiceIPTVResponse("409", "CFS already exists/Duplicate entry", Instant.now().toString(), subscriptionName, "ONT" + request.getOntSN());
                    responses.add(response);
                    continue;
                } else {
                    cfs = new Service();
                ...
                if (optRFS.isPresent()) {
                    ...
                    CreateServiceIPTVResponse response=new CreateServiceIPTVResponse("409", "RFS already exists/Duplicate entry", Instant.now().toString(), subscriptionName, "ONT" + request.getOntSN());
                    responses.add(response);
                    continue;
                } else {
                    rfs = new Service();
                ...
                ontDevice = logicalDeviceRepository
                        ...
                        .get();
                rfs = serviceCustomRepository.findByDiscoveredName(rfsName).get();
                ...
                CreateServiceIPTVResponse response= new CreateServiceIPTVResponse(
                        "201",
